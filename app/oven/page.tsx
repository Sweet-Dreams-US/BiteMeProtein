...
/**
 * /oven — the standalone "tap the oven, get a random treat" page.
 *
 * Restored from git (commit c79850b) on Cole's request. It had been
 * turned into a redirect to /quiz when the oven was merged into the
 * quiz flow, but Cole wanted BOTH: the oven still bakes inside the
 * quiz (personalized match) AND this standalone page lives on as the
 * playful random-reveal toy. Two ways to discover a treat.
 *
 * This page's oven is intentionally its own inline implementation, not
 * the <Oven /> component — that component is a progress-driven
 * cooking/open machine built for the quiz. This one is a simple
 * tap → bake spinner → random reveal interaction.
 */
